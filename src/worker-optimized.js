    ...
    static validateUrl(url) {
        try {
            ...
            return CONFIG.ALLOWED_PROTOCOLS.includes(parsedUrl.protocol) &&
                   !parsedUrl.hostname.match(/^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.|127\.|localhost$|0\.0\.0\.0$)/);
        } catch {
            return false;
        }
    }
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="form-group">
                ...
                <input type="url" id="rssUrl" placeholder="https://example.com/rss.xml" maxlength="500">
            </div>
            <div class="form-group">
...
async function addRssSource(request, env, cache) {
    try {
        ...
        const sanitizedUrl = SecurityUtils.sanitizeInput(url, 500);
        const sanitizedName = SecurityUtils.sanitizeInput(name, 100);
...
async function checkRssUpdates(env) {
    try {
        ...
        const checkTasks = sources.map(source => 
            checkSingleRssSource(source, telegramConfig)
        );
